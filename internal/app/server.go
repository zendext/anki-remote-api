...
import (
    ...
    "github.com/zendext/anki-remote-api/internal/ankiconnect"
)
...
func NewServer(cfg Config) *Server {
    ...
    s := &Server{
        cfg:    cfg,
        anki:   ankiconnect.New(cfg.AnkiConnectURL, 5*time.Second),
        router: r,
    }
    s.routes()
    return s
...
func (s *Server) routes() {
    ...
    s.router.POST("/anki/version", s.handleVersion)
    s.router.POST("/anki/deck-names", s.handleDeckNames)
}
...
func (s *Server) handleVersion(c *gin.Context) {
    version, err := s.anki.Version(c.Request.Context())
    if err != nil {
        c.JSON(http.StatusBadGateway, gin.H{"error": err.Error()})
        return
    }
    c.JSON(http.StatusOK, gin.H{"result": version})
}

func (s *Server) handleDeckNames(c *gin.Context) {
    decks, err := s.anki.DeckNames(c.Request.Context())
    if err != nil {
        c.JSON(http.StatusBadGateway, gin.H{"error": err.Error()})
        return
    }
    c.JSON(http.StatusOK, gin.H{"result": decks})
}
